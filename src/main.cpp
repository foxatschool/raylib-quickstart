...
void InitGame(void)
{
    ...
    player.rotation = 0;
    player.collider = Vector3{ static_cast<float>(player.position.x + sin(player.rotation * DEG2RAD) * (shipHeight / 2.5f), player.position.y - cos(player.rotation * DEG2RAD) * (shipHeight / 2.5f), 12 )};
    ...
    for (int i = 0; i < MAX_BIG_METEORS; i++)
    {
        ...
        bigMeteor[i].position = Vector2{static_cast<float>( posx, posy )};
        ...
        bigMeteor[i].speed = Vector2{static_cast<float>( velx, vely )};
        ...
        if (!pause)
        {
            ...
            if (IsKeyPressed(KEY_SPACE))
            {
                for (int i = 0; i < PLAYER_MAX_SHOOTS; i++)
                {
                    if (!shoot[i].active)
                    {
                        shoot[i].position = Vector2{static_cast<float>( player.position.x + sin(player.rotation * DEG2RAD) * (shipHeight), player.position.y - cos(player.rotation * DEG2RAD) * (shipHeight)) };
                        shoot[i].active = true;
                        ...
                        if (bigMeteor[a].active && CheckCollisionCircles(shoot[i].position, shoot[i].radius, bigMeteor[a].position, bigMeteor[a].radius))
                        {
                            ...
                            for (int j = 0; j < 2; j++)
                            {
                                if (midMeteorsCount % 2 == 0)
                                {
                                    ...
                                    mediumMeteor[midMeteorsCount].speed = Vector2{static_cast<float>( cos(shoot[i].rotation * DEG2RAD) * METEORS_SPEED * -1, sin(shoot[i].rotation * DEG2RAD) * METEORS_SPEED * -1 )};
                                }
                                else
                                {
                                    mediumMeteor[midMeteorsCount].position = Vector2{ bigMeteor[a].position.x, bigMeteor[a].position.y };
                                    mediumMeteor[midMeteorsCount].speed = Vector2{static_cast<float>( cos(shoot[i].rotation * DEG2RAD) * METEORS_SPEED, sin(shoot[i].rotation * DEG2RAD) * METEORS_SPEED) };
                                }
                        ...
                        if (mediumMeteor[b].active && CheckCollisionCircles(shoot[i].position, shoot[i].radius, mediumMeteor[b].position, mediumMeteor[b].radius))
                        {
                            ...
                            for (int j = 0; j < 2; j++)
                            {
                                if (smallMeteorsCount % 2 == 0)
                                {
                                    ...
                                    smallMeteor[smallMeteorsCount].speed = Vector2{static_cast<float>( cos(shoot[i].rotation * DEG2RAD) * METEORS_SPEED * -1, sin(shoot[i].rotation * DEG2RAD) * METEORS_SPEED * -1 )};
                                }
                                else
                                {
                                    smallMeteor[smallMeteorsCount].position = Vector2{ mediumMeteor[b].position.x, mediumMeteor[b].position.y };
                                    smallMeteor[smallMeteorsCount].speed = Vector2{static_cast<float>( cos(shoot[i].rotation * DEG2RAD) * METEORS_SPEED, sin(shoot[i].rotation * DEG2RAD) * METEORS_SPEED) };
                                }
